...
function calculate() {
	...
	if (rightPup < 17) {
		showResult(view.rightRes, 'Smart I<br>Chan 5')
	}

	const leftPup = parseInt(view.leftPupil.value)
	if (leftPup < 17) {
		showResult(view.leftRes, 'Smart I<br>Chan 5')
	}

	if (Math.abs(rightSe - leftSe) < 300) {
		if (rightPup >= 17) {
			const rightAdd = Math.round(parseFloat(view.rightAdd.value) * 100)
			const rightRes = calculateOptic(rightSe, rightAdd, rightPup)
			showResult(view.rightRes, rightRes)
		}
		if (leftPup >= 17) {
			const leftAdd = Math.round(parseFloat(view.leftAdd.value) * 100)
			const leftRes = calculateOptic(leftSe, leftAdd, leftPup)
			showResult(view.leftRes, leftRes)
		}
	}
	else {
		if (rightPup >= 17) {
			showResult(view.rightRes, 'Smart F<br>Chan 7')
			showResult(view.leftRes,  'Smart F<br>Chan 7')
		}
	}
}
